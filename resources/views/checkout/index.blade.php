...
<div class="container text-left">
    ...
    @if (Cart::instance('default')->content()->count() == 0)
        ...
        <h3>Your cart is empty , Please add some product...</h3>
        <a href="{{url('/')}}" class="btn btn-sm btn-dark">Go to shop</a>
    @else
        ...
        <form id="payment-form" method="POST" action="{{route('checkout.store')}}">
            ...
            <h4>Your Order</h4>
                ...
                <table class="table your-order-table">
                    ...
                    <tr>
                        <td><img src="{{url('/assets/uploads').'/'.$item->model->productimage}}" alt="" style="width: 4em"></td>
                        <td>
                            <strong>{{$item->model->productname}}</strong><br>
                            {{$item->model->productdescription}} <br> 
                            <span class="text-dark">$ {{$item->model->productprice}}</span>
                        </td>
                        <td>
